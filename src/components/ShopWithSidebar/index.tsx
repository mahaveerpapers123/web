...
const ShopWithSidebar = () => {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      ...
      setError(null);
      try {
        const searchQuery = typeof window !== "undefined" ? localStorage.getItem("searchQuery") : null;
        const params = new URLSearchParams();
        params.set("page", String(currentPage));
        params.set("limit", String(pagination.limit));
        if (selectedCategory && selectedCategory !== "all") params.set("category", selectedCategory);
        if (searchQuery) params.set("q", searchQuery);
        const res = await fetch(`${API_BASE}/api/products?${params.toString()}`, { credentials: "include", headers: { Accept: "application/json" } });
        const data = await res.json();
        ...
        if (searchQuery) localStorage.removeItem("searchQuery");
      } catch (err: any) {
    ...
    loadProducts();
  }, [API_BASE, currentPage, pagination.limit, selectedCategory]);
  ...
  const pushWithParams = (next: Record<string, string | number | undefined>) => {
  ...
  const handlePageChange = (newPage: number) => {
    if (newPage >= 1 && newPage <= pagination.totalPages) pushWithParams({ page: newPage });
  };

  const handleCategoryChange = (value: string) => {
    setSelectedCategory(value);
    pushWithParams({ category: value, page: 1 });
  };
  ...
  const generatePageNumbers = () => {
    ...
    for (const p of pages) {
      if (lastPage !== undefined && p - lastPage > 1) pagesWithEllipsis.push("...");
  ...
  const startIdx = (pagination.page - 1) * pagination.limit + (products.length ? 1 : 0);
        ...
        <div className="max-w-[1170px] w-full mx-auto px-4 sm:px-8 xl:px-0">
          <div className="flex gap-7.5">
            <div className={`sidebar-content fixed xl:z-1 z-9999 left-0 top-0 xl:translate-x-0 xl:static max-w-[310px] xl:max-w-[270px] w-full ease-out duration-200 ${productSidebar ? "translate-x-0 bg-white p-5 h-screen overflow-y-auto" : "-translate-x-full"}`}>
              ...
              >
                <svg className="fill-current" width="24" height="24" viewBox="0 0 24 24"><path fillRule="evenodd" clipRule="evenodd" d="M10.0068 3.44714C10.3121 3.72703 10.3328 4.20146 10.0529 4.5068L5.70494 9.25H20C20.4142 9.25 20.75 9.58579 20.75 10C20.75 10.4142 20.4142 10.75 20 10.75H4.00002C3.70259 10.75 3.43327 10.5742 3.3135 10.302C3.19374 10.0298 3.24617 9.71246 3.44715 9.49321L8.94715 3.49321C9.22704 3.18787 9.70147 3.16724 10.0068 3.44714Z"/><path fillRule="evenodd" clipRule="evenodd" d="M20.6865 13.698C20.5668 13.4258 20.2974 13.25 20 13.25L4.00001 13.25C3.5858 13.25 3.25001 13.5858 3.25001 14C3.25001 14.4142 3.5858 14.75 4.00001 14.75L18.2951 14.75L13.9472 19.4932C13.6673 19.7985 13.6879 20.273 13.9932 20.5529C14.2986 20.8328 14.773 20.8121 15.0529 20.5068L20.5529 14.5068C20.7539 14.2876 20.8063 13.9703 20.6865 13.698Z"/></svg>
              </button>
              <form onSubmit={(e) => e.preventDefault()}>
                <div className="flex flex-col gap-6">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          pushWithParams({ category: "all", page: 1 });
                        }}
                      >
                        Clean All
                      </button>
                    ...
                    <div className="space-y-2 max-h-72 overflow-y-auto pr-2">
                      {mergedCategories.map((c, i) => (
                        <button
                          ...
                          className={`w-full text-left py-2 px-3 rounded-md border ${selectedCategory === c.value ? "bg-blue text-white border-blue" : "bg-white text-dark border-gray-3 hover:bg-blue hover:text-white hover:border-blue"} duration-200`}
                        >
                          {c.label}
                        </button>
                      ))}
                  ...
                  <GenderDropdown genders={[{ name: "Men", products: 0 }, { name: "Women", products: 0 }, { name: "Unisex", products: 0 }]} />
                  <SizeDropdown />
                  <ColorsDropdwon />
                  <PriceDropdown />
                </div>
              ...
              <div className="rounded-lg bg-white shadow-1 pl-3 pr-2.5 py-2.5 mb-6">
                <div className="flex items-center justify-between">
                  <div className="flex flex-wrap items-center gap-4">
                    <p>
                      Showing <span className="text-dark">{startIdx || 0}–{endIdx}</span> of <span className="text-dark">{pagination.total}</span> Products
                    </p>
                  </div>
