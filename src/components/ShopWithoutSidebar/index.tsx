...
const normalizeItem = (p: any): Product & { imgs?: { previews?: string[]; thumbnails?: string[] } } => {
  const fromImages = Array.isArray(p.images) ? p.images : [];
  const fromThumbs = p?.imgs?.thumbnails ?? [];
  const fromPrev = p?.imgs?.previews ?? [];
  const merged = [...fromThumbs, ...fromPrev, ...fromImages].filter(Boolean);
  const https = merged.map((u: string) => (u?.startsWith("http://") ? u.replace("http://", "https://") : u));
  return {
    ...p,
    images: fromImages.length ? https : https,
    imgs: {
      thumbnails: https,
      previews: https,
    },
...
const ShopWithoutSidebar = () => {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      ...
      try {
        ...
        const data = await shopData(currentPage, pagination.limit, searchQuery || undefined);
        ...
        setPagination((prev) => ({
          ...
          page: data.page,
          total: data.total,
          totalPages: Math.max(1, Math.ceil(data.total / prev.limit)),
        }));
  ...
  const generatePageNumbers = () => {
    ...
    for (const p of pages) {
      if (last !== undefined && p - last > 1) pagesWithEllipsis.push("...");
  ...
  const title =
    (typeof window !== "undefined" && localStorage.getItem("searchQuery")) || "Explore All Products";
